...
def test_telegram():
    ...
    return redirect(url_for("admin_panel"))
@app.route("/test-betfair-connection")
@login_required
def test_betfair_connection():
    """Test Betfair API connection and show detailed results"""
    if not current_user.is_admin:
        flash("Admin only", "danger")
        return redirect(url_for("dashboard"))
    ...
    results.append("<h2>Step 1: Environment Variables</h2>")
    env_vars = {
    ...
    for key, value in env_vars.items():
        if value:
            ...
            results.append(f"<p>✅ {key}: {masked}</p>")
        else:
            results.append(f"<p>❌ {key}: NOT SET</p>")
    
    # Step 2: Test Betfair login
    results.append("<h2>Step 2: Betfair Login Test</h2>")
    
    try:
        from betfair_service import BetfairService
        
        betfair = BetfairService()
        results.append(f"<p>ℹ️ BetfairService initialized</p>")
        results.append(f"<p>ℹ️ Proxy configured: {bool(betfair.proxies)}</p>")
        
        results.append("<p>🔄 Attempting login...</p>")
        login_success = betfair.login()
        
        if login_success:
            results.append("<p>✅ <strong>Login SUCCESSFUL!</strong></p>")
            results.append(f"<p>Session Token: {betfair.session_token[:20]}...</p>")
        else:
            results.append("<p>❌ <strong>Login FAILED</strong></p>")
            results.append("<p>Check Railway logs for details</p>")
    except Exception as e:
        results.append(f"<p>❌ <strong>Login Error:</strong> {str(e)}</p>")
        import traceback
        results.append(f"<pre>{traceback.format_exc()}</pre>")
    
    # Step 3: Test market search
    results.append("<h2>Step 3: Market Search Test</h2>")
    
    try:
        from betfair_service import BetfairService, parse_meeting_name
        
        today = datetime.now().strftime('%y%m%d')
        test_meeting = f"{today}_Caulfield"
        
        results.append(f"<p>Testing with meeting: {test_meeting}</p>")
        
        meeting_date, track_name = parse_meeting_name(test_meeting)
        results.append(f"<p>Parsed date: {meeting_date}</p>")
        results.append(f"<p>Parsed track: {track_name}</p>")
        ...
        if meeting_date and track_name:
            ...
            if betfair.session_token:
                results.append("<p>🔄 Searching for markets...</p>")
                markets = betfair.find_markets_for_meeting(meeting_date, track_name)
                
                if markets:
                    results.append(f"<p>✅ <strong>Found {len(markets)} markets!</strong></p>")
                    results.append("<ul>")
                    for market in markets[:5]:
                        event = market.get('event', {})
                        results.append(f"<li>{event.get('name')} - {market.get('marketName')}</li>")
                    results.append("</ul>")
                else:
                    results.append("<p>⚠️ No markets found</p>")
                    results.append("<p>Possible reasons:</p>")
                    results.append("<ul>")
                    results.append(f"<li>No races at {track_name} today</li>")
                    results.append("<li>Track name mismatch (check TRACK_MAPPING)</li>")
                    results.append("<li>Markets not open yet</li>")
                    results.append("</ul>")
            else:
                results.append("<p>❌ Cannot search - not logged in</p>")
        else:
            results.append("<p>❌ Failed to parse meeting name</p>")
            
    except Exception as e:
        results.append(f"<p>❌ <strong>Market Search Error:</strong> {str(e)}</p>")
        import traceback
        results.append(f"<pre>{traceback.format_exc()}</pre>")
    
    # Step 4: Test with recent meeting
    results.append("<h2>Step 4: Test with Recent Meeting</h2>")
    
    try:
        recent_meeting = Meeting.query.order_by(Meeting.uploaded_at.desc()).first()
        
        if recent_meeting:
            results.append(f"<p>Testing with: {recent_meeting.meeting_name}</p>")
            
            from betfair_service import BetfairService, parse_meeting_name
            
            meeting_date, track_name = parse_meeting_name(recent_meeting.meeting_name)
            results.append(f"<p>Parsed date: {meeting_date}</p>")
            results.append(f"<p>Parsed track: {track_name}</p>")
            ...
            if betfair.session_token and meeting_date and track_name:
                markets = betfair.find_markets_for_meeting(meeting_date, track_name)
                
                if markets:
                    results.append(f"<p>✅ Found {len(markets)} markets</p>")
                    
                    matched = 0
                    for race in recent_meeting.races[:3]:
                        market_id = betfair.match_race_to_market(race.race_number, markets)
                        if market_id:
                            results.append(f"<p>✅ Race {race.race_number} → Market ID: {market_id}</p>")
                            matched += 1
                        else:
                            results.append(f"<p>⚠️ Race {race.race_number} → No match</p>")
                    
                    results.append(f"<p><strong>Matched {matched} out of {min(3, len(recent_meeting.races))} races</strong></p>")
                else:
                    results.append("<p>⚠️ No markets found for this meeting</p>")
        else:
            results.append("<p>No meetings in database</p>")
            
    except Exception as e:
        results.append(f"<p>❌ <strong>Database Test Error:</strong> {str(e)}</p>")
        import traceback
        results.append(f"<pre>{traceback.format_exc()}</pre>")
    ...
    <html>
    <head>
        <title>Betfair Connection Test</title>
        <style>
    ...
    <body>
        <h1>🔍 Betfair API Connection Diagnostic</h1>
        {}
        <hr>
