...
def ensure_component_analysis_table():
    """Create the nightly Component Analysis cache table if it doesn't exist yet."""
    with engine.connect() as conn:
        conn.execute(text("""
        ...
        """))
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS ix_component_analysis_cache_computed_at
            ON component_analysis_cache (computed_at DESC)
        """))
        conn.commit()
    log.info("component_analysis_cache table verified.")
...
def extract_features(row, jockey_sr_lookup=None, trainer_sr_lookup=None,
    ...
    if rating:
        ...
        # Same key INSERTION ORDER as the rating-present branch above (incl.
        # pf_is_reliable's position): pd.DataFrame(list-of-dicts) derives its
        # column order from the first row's key order, so the two branches
        # diverging would make the training column order depend on whether the
        # first extracted row happened to have a ratings entry.
...
def _live_scoring_feature_names():
    """Ordered feature names the LIVE scorer (ml_predict.py) can generate.

    This is the other half of the feature contract: training columns matching
    the artifact is necessary but not sufficient — a candidate whose features
    live scoring cannot compute would pass every training-side check and then
    fail (or median-fill its new signal away) on every real meeting. Returns
    None when ml_predict can't be imported (log-and-skip rather than fail the
    whole nightly run on an import quirk).
    """
    try:
        from ml_predict import FEATURE_NAMES as live_feature_names
        return [str(name) for name in live_feature_names]
    except Exception as e:
        log.warning(
            "Could not import the live scoring feature contract from ml_predict "
            "(%s); skipping the live-generatability check.", e,
        )
        return None


def _stored_feature_list(model):
    """The feature list persisted on an artifact, or None when it has none.

    An artifact with no (or an empty) stored feature list cannot score a
    single live race: ml_predict._assert_live_feature_contract raises
    "ML feature contract failed: model artifact has no persisted feature
    list" before it ever reaches predict(). Treat missing and empty
    identically — both are equally unusable in production.
    """
...
def _artifact_feature_contract_ok(model, feature_names):
    """True when the artifact's stored features match this run's training
    columns AND live scoring can actually generate every one of them.

    Live generation is checked as a set (live may lawfully generate a
    superset, and inference reindexes to the artifact's stored order), while
    the artifact-vs-training check stays exact-order.
    """
    stored = getattr(model, 'feature_names_in_', None)
    if stored is None:
        stored = getattr(model, '_form_analyst_expected_features', None)
    ...
    if [str(x) for x in list(stored)] != feature_names:
        return False
    live_names = _live_scoring_feature_names()
    if live_names is None:
        return True
    not_live_computable = [name for name in feature_names if name not in set(live_names)]
    if not_live_computable:
        log.error(
            "Feature contract failure: %s training feature(s) cannot be generated by live "
            "scoring (ml_predict.py) and would be silently median-filled on every real "
            "meeting: %s. Wire them into ml_predict.extract_features before letting a "
            "model trained on them compete for champion.",
            len(not_live_computable), not_live_computable,
        )
        return False
    return True
...
def save_best_model_to_db(pkl_file, combined_score, run_id, model_type='random_forest',
    ...
    # An artifact with no persisted feature list is unusable in production no
    # matter how it scores, so it can never be promoted through this path.
    challenger_feature_list_missing = _stored_feature_list(saved_model) is None

    # Promotion-time half of the feature contract: every feature this artifact
    # was trained on must be generatable by live scoring (ml_predict.py), or a
    # "better" challenger would win the competition and then have its new
    # signal silently median-filled on every real meeting. main() already
    # blocks such candidates via _artifact_feature_contract_ok, but this gate
    # protects the actual promotion decision for artifacts saved through any
    # other path.
    live_feature_names = _live_scoring_feature_names()
    not_live_computable = []
    if live_feature_names is not None and saved_features is not None:
        live_set = set(live_feature_names)
        not_live_computable = [str(x) for x in list(saved_features) if str(x) not in live_set]
    ...
    with engine.connect() as conn:
        ...
        if challenger_feature_list_missing:
            ...
            # reads as "passed" for an artifact that cannot score a race at all.
            reason = (
                "Rejected: artifact has no persisted feature list and cannot generate live predictions "
                "(ml_predict raises 'model artifact has no persisted feature list' on every race)"
            )
        elif not_live_computable:
            reason = (
                f"Rejected: {len(not_live_computable)} trained feature(s) cannot be generated by live "
                f"scoring (ml_predict.py) and would be median-filled on every real meeting: "
                f"{not_live_computable[:10]}"
            )
...
def rollback_to_champion(model_id, reason='Manual Champion rollback'):
    ...
    with engine.connect() as conn:
        ...
        # Last line of defence on the activation itself. can_become_champion and
        # _assert_champion_comparable both read selection_metrics only — neither
        # ever opens the artifact — so without this check a model with no
        # persisted feature list could still be made active through a direct or
        # manual rollback call, which is exactly how Champion 79 was promoted.
        # No model that cannot score a live race may be activated by any path.
        artifact_row = conn.execute(text("""
            SELECT pkl_data FROM backtest_best_model WHERE id = :id
        """), {'id': model_id}).fetchone()
        if not artifact_row or not artifact_row[0]:
            raise ValueError(f"Model {model_id} has no stored artifact and cannot be activated")
        try:
            target_model = joblib.load(io.BytesIO(artifact_row[0]))
        except Exception as e:
            raise ValueError(f"Model {model_id} artifact failed to load and cannot be activated: {e}")
        if _stored_feature_list(target_model) is None:
            raise ValueError(
                f"Model {model_id} has no persisted feature list and cannot generate live predictions "
                "(ml_predict would raise 'model artifact has no persisted feature list' on every race). "
                "Refusing to activate it as champion."
            )
...
def _replace_unusable_champion(conn, champion_id, current_features, is_active, run_id=None):
    """Evict an active champion that cannot generate live predictions at all.

    Deliberately NOT score-gated. Everywhere else in this pipeline a sitting
    champion is only displaced by something that outscores it by
    PROMOTION_SELECTION_SCORE_EDGE. That rule is wrong here, because it
    silently assumes the incumbent is worth something: a champion with no
    persisted feature list produces ZERO predictions on every race, so any
    eligible model with an intact feature contract is strictly better than it
    no matter how the two Champion Scores compare. Applying the usual edge
    bar is what let Champion 79 survive run 172 — its recomputed score of
    -13.601 simply happened to be higher than everything else on the shelf.

    Order of preference:
      1. The best-scoring rejected challenger with an intact feature contract
         (score is used only to RANK usable candidates, never as a bar to clear).
      2. If none exists: deactivate outright and flag permanently_incompatible,
         leaving no active champion. Live scoring is already returning zero
         predictions in this state, so nothing is lost by making the DB honest
         about it — and save_best_model_to_db treats permanently_incompatible
         as "no active champion", so the next validated challenger promotes on
         its own merits with no bar to clear.
    """
    log.error(
        "Active champion id=%s has NO persisted feature list — it cannot score any live race "
        "(ml_predict raises 'model artifact has no persisted feature list' on every meeting). "
        "Evicting it regardless of its Champion Score.", champion_id,
    )
    ...
    if best_challenger and is_active:
        reason = (
            f"Unusable-champion eviction: champion {champion_id} has no persisted feature list and cannot "
            f"generate live predictions. Replaced by challenger {best_challenger['id']} "
            f"(Champion Score {best_challenger['score']:.3f}), which has an intact feature contract. "
            "Not score-gated: any usable model beats an unusable champion."
        )
        log.info(reason)
        # Same reason as the self-heal rollback path: _best_rejected_challenger
        # only recomputed this candidate's score in memory, so its stored row
        # may still carry a stale/missing scoring_formula_version that
        # rollback_to_champion's own comparability re-check would reject.
        ...
        return {
            'healed': True,
            'unusable_champion': True,
            'rolled_back_to': best_challenger['id'],
            'detail': (
                f"champion id={champion_id} had no persisted feature list and was replaced by "
                f"challenger id={best_challenger['id']} (Champion Score {best_challenger['score']:.3f})."
            ),
        }

    # No usable replacement anywhere — deactivate rather than leave a champion
    # active that is guaranteed to fail on every race.
    ...
    if champion_row and champion_row[0]:
        try:
            existing_metrics = json.loads(champion_row[0])
        except Exception:
            existing_metrics = {}
    flagged_metrics = {
        **existing_metrics,
        'permanently_incompatible': True,
        'unusable_missing_feature_list': True,
    }
    conn.execute(text("""
        UPDATE backtest_best_model
        SET is_active = FALSE,
        ...
        WHERE id = :id
    """), {
        'metrics': json.dumps(flagged_metrics),
        'retention_days': CHAMPION_ROLLBACK_RETENTION_DAYS,
        'id': champion_id,
    })
    conn.commit()
    log.error(
        "Champion id=%s DEACTIVATED: no persisted feature list and no usable replacement available. "
        "There is now no active champion. Live scoring was already producing zero predictions in this "
        "state; the next validated challenger will be promoted on its own merits with no comparison bar.",
        champion_id,
    )
    return {
        'healed': False,
        ...
        'detail': (
            f"champion id={champion_id} has no persisted feature list and cannot generate live predictions. "
            "No rejected challenger with an intact feature contract was available to replace it, so it was "
            "deactivated and flagged permanently_incompatible. There is currently no active champion."
        ),
    }
...
def _heal_stale_champion(champion_id, champion_metrics, run_id=None):
    ...
    with engine.connect() as conn:
        ...
        expected_features = _stored_feature_list(model)
        if expected_features is None:
            # HARD INVARIANT: a champion with no persisted feature list cannot
            # score a single live race (ml_predict raises before predict()).
            # This is disqualifying ON ITS OWN — it is emphatically NOT a
            # "re-score it and see if anything beats it" situation, which is
            # what used to happen here: the old guard read
            #   if expected_features is not None and list(...) != list(...)
            # so a MISSING list skipped the permanent-incompatibility branch
            # entirely, fell through to the re-scoring path below, and got
            # re-stamped with the current scoring_formula_version every night
            # — laundering an unusable champion into looking freshly
            # validated. Champion 79 sat active for days that way.
            return _replace_unusable_champion(
                conn, champion_id, list(X.columns), is_active, run_id=run_id,
            )

        if list(expected_features) != list(X.columns):
            return {
                'healed': False,
                'permanent': True,
                'detail': (
...
def check_active_champion_staleness(run_id=None):
    ...
    with engine.connect() as conn:
        if result['healed']:
            ...
            if result.get('permanent') and not result.get('already_flagged'):
...
# ─────────────────────────────────────────────
# COMPONENT ANALYSIS (nightly, all history)
...
def run_full_component_analysis(df):
    ...
    # Top pick per race — needed for the raw-factor mining pass below (E)
    # so it can accumulate top-pick-only stats (F2) in the same loop.
    top_picks = {}
    for race_id, horses in races.items():
        if horses:
    ...
    #     Also feeds F2 (top-pick-only raw factors) via the same pass —
    #     see _current_is_top_pick below.
    # ══════════════════════════════════════════════════════════════════
    ...
    total_raw = len(df)
    raw_wins  = int((df['finish_position'] == 1).sum())
    ...
    for row in df.itertuples(index=False):
        ...
        track_cond = row.track_condition if isinstance(row.track_condition, str) else ''
        meeting_name = row.meeting_name if isinstance(row.meeting_name, str) else ''

        _current_is_top_pick[0] = (top_picks.get(race_id) == horse_name)
        ...
        pm_match  = re.search(r"\$([\d,]+)", str(prize_raw))
        try:
            if pm_match:
        ...
        try:
            m2 = re.match(r"^(\d+)", str(fsize_raw).strip())
            if m2:
        ...
        sect_match = re.match(r"([\d.]+)sec", sect_raw)
        try:
            if sect_match:
    ...
    #         accumulated above in the same pass as E, via tp_raw_buckets.
    # ══════════════════════════════════════════════════════════════════
    ...
    log.info(
        "Full component analysis complete: %d components, %d raw factors, %d races.",
        len(components_list), len(raw_factors), len(races),
    )
    ...
    return {
        ...
        'raw_meta':        raw_meta,
        'tp_notes':        tp_notes_list,
        'tp_raw':          tp_raw_list,
    }


def write_component_analysis_cache(payload, run_id):
    with engine.connect() as conn:
        conn.execute(text("""
            INSERT INTO component_analysis_cache (run_id, payload, computed_at)
            VALUES (:run_id, :payload, NOW())
        """), {'run_id': run_id, 'payload': json.dumps(payload)})
        conn.execute(text("""
            DELETE FROM component_analysis_cache
            WHERE id NOT IN (
                SELECT id FROM component_analysis_cache ORDER BY computed_at DESC LIMIT 5
            )
        """))
        conn.commit()
    log.info("Wrote component analysis cache (run_id=%s).", run_id)
...
def main():
    ...
    ensure_tables()
    ensure_component_analysis_table()
