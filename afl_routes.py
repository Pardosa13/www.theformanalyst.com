...
def register_afl_routes(app, db):
    ...
    def api_afl_player_stats():
        ...
        rows = _db_player_search(db, name=name, team=team, season=effective_season, limit=max(limit * 30, 300))
        if not rows:
            return jsonify({"players": [], "season": effective_season, "stat": stat, "message": "No players found"})
        ...
        result = result[:limit]
        return jsonify({"players": result, "season": effective_season, "stat": stat, "count": len(result)})
    ...
    def api_afl_player_vs_opponent():
        ...
        rows = _db_player_vs_opponent(db, player_id=player_id, name=name, opponent=opponent, season_from=season_from)
        if not rows:
            return jsonify({"games": 0, "opponent": opponent, "averages": {}, "hit_rates": {}, "game_log": []})
    ...
    def api_afl_team_players():
        ...
        rows = _db_player_search(db, team=team, season=None, limit=max(limit * 40, 400))
        if not rows:
            return jsonify({"team": team, "players": []})
        grouped = _group_players(rows)
        players = []
        ...
        return jsonify({"team": team, "stat": stat, "players": players[:limit], "count": min(len(players), limit)})
    ...
    def api_afl_value_finder():
        ...
        if not props:
            return jsonify({"bets": [], "message": "No prop lines loaded. Configure The Odds API key.", "season": effective_season})
        ...
        for prop in props:
            ...
            opp_rows = [r for r in player_rows if r.get("match_home_team") == opponent or r.get("match_away_team") == opponent]
            ...
            edge_data = calculate_disposal_edge(player_avg=season_avg, book_line=book_line, vs_opp_avg=vs_opp_avg, last5_avg=last5_avg)
            if abs(edge_data["edge"]) >= min_edge or edge_data["recommendation"] == "value":
                ...
                value_bets.append({
                    "player": player_name, "team": team_name, "opponent": opponent,
                    "home_team": home_team, "away_team": away_team,
                    "commence_time": str(prop.get("commence_time", "")),
                    "bookmaker": prop.get("bookmaker", ""), "market": market,
                    "book_line": book_line, "over_odds": prop.get("odds", 0),
                    "season_avg": season_avg, "vs_opp_avg": vs_opp_avg,
                    "last5_avg": last5_avg, "hist_pct": hist_pct, **edge_data,
                })
        value_bets.sort(key=lambda x: abs(x.get("edge", 0)), reverse=True)
        return jsonify({"market": market, "min_edge": min_edge, "season": effective_season, "bets": value_bets, "count": len(value_bets)})
...
def _db_current_round(db, year: int) -> int:
    sql = db.text("SELECT COALESCE(MAX(round), 1) FROM afl_games WHERE year = :year AND complete = 100")
...
def _db_next_game(db, year: int) -> dict:
    sql = db.text("SELECT hteam, ateam, date, venue FROM afl_games WHERE year = :year AND complete < 100 ORDER BY date ASC LIMIT 1")
    with db.engine.connect() as conn:
    ...
    return {"time": time_str, "teams": f"{row['hteam']} vs {row['ateam']}", "venue": row["venue"]}
...
def _db_player_search(db, name: str = "", team: str = "", season: int = None, limit: int = 50) -> list[dict]:
    ...
    params = {"limit": limit}
    if name:
        conditions.append("(LOWER(player_first_name || ' ' || player_last_name) LIKE :name OR LOWER(player_last_name) LIKE :name_last)")
    ...
    sql = db.text(f"SELECT * FROM afl_player_stats {where} ORDER BY match_date DESC LIMIT :limit")
    with db.engine.connect() as conn:
...
def _db_player_by_id(db, player_id: int, season: int = None, limit: int = 100) -> list[dict]:
    ...
    sql = db.text(f"SELECT * FROM afl_player_stats WHERE player_id = :player_id {season_filter} ORDER BY match_date DESC LIMIT :limit")
    with db.engine.connect() as conn:
        rows = conn.execute(sql, params).mappings().fetchall()
    return [dict(r) for r in rows]
...
def _db_player_vs_opponent(db, player_id: int = None, name: str = "", opponent: str = "", season_from: int = 2019) -> list[dict]:
    ...
    sql = db.text(f"""
        ...
        WHERE season >= :season_from
          AND (LOWER(match_home_team) LIKE LOWER(:opp) OR LOWER(match_away_team) LIKE LOWER(:opp))
          {id_filter}
...
def _db_get_fixtures(db, year: int, round_number: int = None) -> list[dict]:
    ...
    sql = db.text(f"SELECT g.* FROM afl_games g WHERE g.year = :year AND g.complete < 100 {round_filter} ORDER BY g.date ASC LIMIT 50")
    with db.engine.connect() as conn:
        rows = conn.execute(sql, params).mappings().fetchall()
    return [dict(r) for r in rows]
...
def _db_get_standings(db, year: int, round_number: int = None) -> list[dict]:
    ...
    sql = db.text(f"SELECT * FROM afl_standings WHERE year = :year {round_filter} ORDER BY rank ASC")
    with db.engine.connect() as conn:
        rows = conn.execute(sql, params).mappings().fetchall()
    return [dict(r) for r in rows]
...
def _db_get_props(db, market: str = "player_disposals") -> list[dict]:
    sql = db.text("""
        ...
        FROM afl_player_props
        WHERE market = :market AND fetched_at > NOW() - INTERVAL '24 hours'
...
def _db_get_match_props(db, home_team: str, away_team: str) -> list[dict]:
    ...
    """)
    with db.engine.connect() as conn:
        rows = conn.execute(sql, {"home": f"%{home_team}%", "away": f"%{away_team}%"}).mappings().fetchall()
    return [dict(r) for r in rows]
...
def _check_data_sources(db) -> dict:
    ...
    return {
        ...
        "odds_api": _has_data("SELECT 1 FROM afl_player_props WHERE fetched_at > NOW() - INTERVAL '24 hours' LIMIT 1"),
    }
...
def _merge_fixture_tips(db, fixtures: list[dict], year: int, round_number: int | None = None) -> list[dict]:
    ...
    for fixture in fixtures:
        ...
        if not tip:
            tip = tips_by_matchup.get((fixture.get("hteam", ""), fixture.get("ateam", ""), fixture.get("round")))
        row = dict(fixture)
        if tip:
