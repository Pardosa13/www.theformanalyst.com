...
# Support Railway/Github env: prefer DATABASE_URL if present
SQLALCHEMY_DATABASE_URI = os.getenv("SQLALCHEMY_DATABASE_URI") or os.getenv("DATABASE_URL")
if not SQLALCHEMY_DATABASE_URI:
    print("Please set SQLALCHEMY_DATABASE_URI or DATABASE_URL environment variable (e.g. postgresql://user:pass@host/db)")
    sys.exit(1)
...
with engine.begin() as conn:
    for k, v in to_add.items():
        sql = f'ALTER TABLE "{table_name}" ADD COLUMN IF NOT EXISTS "{k}" {v};'
        print("Executing:", sql)
